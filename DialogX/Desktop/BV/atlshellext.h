...
public:
   ...
   DWORD GetHash() const
   {
      ...
      for( UINT i = 0; i < nSize; i++ ) dwHash += *pBytes++;
      return dwHash;
   }
...
public:
   ...
   CPidlList(HWND hwndList, DWORD dwListViewMask) : m_pidls(NULL), m_nCount(0)
   {
      ...
      while( (nItem = ListView_GetNextItem(hwndList, nItem, dwListViewMask)) != -1 ) {
         ...
         lvi.mask = LVIF_PARAM;
         lvi.iItem = nItem;
         if( ListView_GetItem(hwndList, &lvi) ) pidls[i++] = CPidl::PidlCopy( reinterpret_cast<LPITEMIDLIST>(lvi.lParam) );
      }
...
public:
   ...
   {
      ATLTRACENOTIMPL(_T("IFolderViewImpl::SetCurrentViewMode"));
   }

   STDMETHOD(GetFolder)(REFIID riid, void** ppv)
   {
   ...
   STDMETHOD(GetFocusedItem)(int* piItem)
   {
      ...
      T* pT = static_cast<T*>(this); pT;
      *piItem = ListView_GetNextItem(pT->m_hwndList, iItem, LVNI_FOCUSED);
      return iItem >= 0 ? S_OK : E_FAIL;
   }
   ...
   STDMETHOD(SelectAndPositionItems)(UINT /*cidl*/, LPCITEMIDLIST* /*apidl*/, POINT* /*apt*/, DWORD /*dwFlags*/)
   {
      ATLTRACENOTIMPL(_T("IFolderViewImpl::SelectAndPositionItems"));
   }
};
