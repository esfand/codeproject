...
public:
   ...
   static BOOL AddJumboIcon(HIMAGELIST hImageList, LPCTSTR pstrPath, UINT uFlags)
   {
      ...
      if( ovi.dwMajorVersion >= 6 ) {
         HINSTANCE hShell32 = ::LoadLibrary(_T("shell32.dll")); // AR
   ...
   static SIZE GetShellIconSize(UINT uType)
   {
      ...
      SHFILEINFO sfi = { 0 };
      ::SHGetFileInfo(_T("temp.txt"), 0, &sfi, sizeof(sfi), SHGFI_ICON | SHGFI_USEFILEATTRIBUTES | SHGFI_SHELLICONSIZE | uType); // AR
      if( sfi.hIcon != NULL ) {
         ICONINFO ii = { 0 };
